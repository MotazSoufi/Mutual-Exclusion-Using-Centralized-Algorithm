...
public class functions {
	...
	public static ArrayList<String> getRandPasswords(int number, int length) { //gen # of passwords with length
		String password = "";
		ArrayList<String> results = new ArrayList<String>(); //for new generated passwords
		Random rand = new Random(); //Random object
		for (int j=0; j<number; j++) { //Depends on # of passwords
			for (int i=0; i<length; i++) {
		    	String randomElement = arrayLetters[rand.nextInt(arrayLetters.length)]; //randomize 
		    	password += randomElement; //append
		    }
			results.add(password); //add new password to reuslts 
			password = ""; //refresh password
		}
		return results; //Exit loop and final generated passwords 
    }
		...
		// The function only takes the array as the argument
